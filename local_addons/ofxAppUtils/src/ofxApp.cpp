...
#ifndef OFX_APP_UTILS_NO_CONTROL_PANEL
	app->controlPanel.setup("App Controls", 1, 0, 1024, 768);
#endif
	app->setup();
}
...
void ofxRunnerApp::draw() {
  /*if (app->use_background) {
    ofSetColor(255, app->backgroundOpacity);
    app->background.draw(0,0,ofGetWidth(), ofGetHeight());
  }
  */
	...
	if(app->_bEditingWarpPoints && app->bDebug) {
		...
		ofDrawRectangle(0.35, 0.35, app->_renderWidth-1.35, app->_renderHeight-1.35);
		ofSetRectMode(OF_RECTMODE_CORNER);
		ofFill();
