...
void ofxControlPanel::loadSettings(string xmlFile){
    if(xmlFile == "") {
        ofFileDialogResult dialog_result = ofSystemLoadDialog("Load settings file", false, defaultConfigurationDirectory);
        if(dialog_result.bSuccess) {
          xmlFile = dialog_result.getPath();
        } else {
          return;
        }
    }
    ...
    for(unsigned int i = 0; i < guiObjects.size(); i++)
      guiObjects[i]->loadSettings(settings);
...
void ofxControlPanel::saveSettings() {
    if(currentXmlFile == "") {
		ofFileDialogResult dialog_result = ofSystemSaveDialog("", "Save settings file");//, defaultConfigurationDirectory);
        if(dialog_result.bSuccess) {
          currentXmlFile = dialog_result.getPath();
        } else {
          return;
        }
    ...
    for(int i = 0; i < (int) guiObjects.size(); i++)
      guiObjects[i]->saveSettings(settings);
