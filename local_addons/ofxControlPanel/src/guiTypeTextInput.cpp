...
void guiTypeTextInput::render()
{
	...
	if( textSelected ){
		...
		ofDrawRectangle( hitArea.x+startPositionX, hitArea.y, endPositionX - startPositionX,  hitArea.height);
	}else{
		// draw blinking cursor
		if( !isLocked() && value.getValueB() && (ofGetElapsedTimeMillis()%500)>250){
			glColor4fv(textColor.getColorF());
			text_position = min(max(0, text_position), (int)valueText.textString.length() );
			ofDrawRectangle( hitArea.x+valueText.getTextWidth(valueText.textString.substr(0,text_position))+1, hitArea.y+1, 2, hitArea.height-2 );
		}
	}
